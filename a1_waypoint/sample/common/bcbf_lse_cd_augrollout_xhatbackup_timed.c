...
#define BCBF_USE_AUGMENTED_TERMINAL_SET 1
#endif
...
static double cbf_log_qp_time_sec = 0.0;   // instantaneous filter computation time [s]
static double cbf_log_filter_time_sec = 0.0; // alias for plotting/comparison [s]
static double cbf_log_total_time_sec  = 0.0; // accumulated filter time [s]
static double cbf_log_avg_time_sec    = 0.0; // running average filter time [s]
static double cbf_log_max_time_sec    = 0.0; // maximum observed filter time [s]
static int    cbf_log_call_count      = 0;   // number of filter calls
...
//            bcbf_cd_u_state[0], bcbf_cd_u_state[1], bcbf_cd_u_state[2]);
    ...
    *wz_cmd = bcbf_cd_u_state[2];
    ...
    // Computation-time logging. Keep the variable name cbf_log_qp_time_sec
    // for compatibility with the previous QP/LSE/closed-form files and
    // plotting scripts, even though this version does not call a QP solver.
    double elapsed_sec = cbf_now_sec() - t0;
    cbf_log_qp_time_sec = elapsed_sec;
    cbf_log_filter_time_sec = elapsed_sec;
    cbf_log_total_time_sec += elapsed_sec;
    cbf_log_call_count += 1;
    cbf_log_avg_time_sec = cbf_log_total_time_sec / (double)cbf_log_call_count;
    if (elapsed_sec > cbf_log_max_time_sec) cbf_log_max_time_sec = elapsed_sec;
