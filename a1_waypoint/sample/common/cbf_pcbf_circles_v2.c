...
//      Positive = safe (outside), negaa, tive = violation (inside).
//
...
static void spline_fit_natural(const double *y, int n, double dt, double *M)
{
    ...
    //   M_{i-1} + 4·M_i + M_{i+1} = 6/dt² · (y_{i-1} - 2·y_i + y_{i+1})
    // (only M[1]..M[n-1] are unknowns)
    int m = n - 1;   // number of unknowns
    double *c_arr = (double*)__builtin_alloca(m * sizeof(double));
    double *d_arr = (double*)__builtin_alloca(m * sizeof(double));
    double *w_arr = (double*)__builtin_alloca(m * sizeof(double));
    ...
    double dV_dx0[N_OBS][3])
{
    ...
    for (int i = 0; i < N_OBS; i++)
    {
        ...
        // temporary debug
        if (i == 0)
            printf("[PCBF] k*=%d tau*=%.3f V=%.4f h0=%.4f\n",
                   k_star, tau_star, V_val[i], h_traj[0][0]);
