...
static double cbf_log_filter_time_sec = 0.0;
                                   ...
                                   double b_cbf[BCBF_N_CBF_ROWS])
{
    ...
    // so they contribute ONE smooth terminal constraint instead of BCBF_N_OBS rows.
    double hB_lse;
    double grad_xB_lse[3];
    bcbf_h_backup_lse_and_gradx(z, Phi, &hB_lse, grad_xB_lse);
    bcbf_push_constraint_row(grad_xB_lse, theta, hB_lse, bcbf_alpha_B,
                             A_cbf[row], &b_cbf[row]);
    row++;
}
