...
static const double bcbf_kappa_lse_B = 20.0;  // larger -> closer to hard min over backup-set margins
static const double bcbf_kappa_lse = 20.0;    // larger -> closer to hard min over all horizon/terminal terms
...
// Input limits are enforced separately as command-state CBF rows; they are not included in the LSE.
#define BCBF_N_BACKUP_LSE_TERMS (BCBF_N_OBS * BCBF_N_SAMPLES + BCBF_N_OBS)
#define BCBF_N_LSE_TERMS BCBF_N_BACKUP_LSE_TERMS
#define BCBF_N_CBF_ROWS  1
#define BCBF_N_INPUT_ROWS 6
#define BCBF_N_ROWS      (BCBF_N_CBF_ROWS + BCBF_N_INPUT_ROWS)
...
static void bcbf_build_constraints(double x, double y, double theta,
                                   ...
                                   double A_cbf[BCBF_N_ROWS][3],
                                   double b_cbf[BCBF_N_ROWS])
{
    ...
    // Note: input-limit terms are NOT included in H_LSE.
    // H_LSE aggregates only backup-flow obstacle terms and terminal backup-set terms.
    ...
    // These are separate QP rows, not part of the LSE backup barrier.
    double A_input[BCBF_N_INPUT_ROWS][3];
    double b_input[BCBF_N_INPUT_ROWS];
    bcbf_push_input_limit_rows(z0, A_input, b_input);
    for (int r = 0; r < BCBF_N_INPUT_ROWS; r++) {
        for (int c = 0; c < 3; c++) A_cbf[BCBF_N_CBF_ROWS + r][c] = A_input[r][c];
        b_cbf[BCBF_N_CBF_ROWS + r] = b_input[r];
    }
}
