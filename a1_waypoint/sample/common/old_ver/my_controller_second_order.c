...
// -----------------------------------------------------------------------------
// High-level velocity estimate for optional second-order/command-tracking model.
// Current safety filters still use the first-order kinematic model unless you
// later switch to a second-order CBF filter. These estimates are logged so we
// can identify the locomotion-stack command-tracking dynamics:
//      qdot = G(theta) nu,
//      nudot ~= Tau^{-1}(u_cmd - nu).
// -----------------------------------------------------------------------------
static int    hl_vel_initialized = 0;
static double hl_prev_x = 0.0;
static double hl_prev_y = 0.0;
static double hl_prev_theta = 0.0;
static double hl_prev_time = 0.0;
static double hl_vx_body_est = 0.0;
static double hl_vy_body_est = 0.0;
static double hl_wz_est = 0.0;

static inline double wrap_to_pi_ctrl(double a)
{
...
static inline void update_highlevel_velocity_estimate(
    double x, double y, double theta, double t)
{
    if (!hl_vel_initialized) {
        hl_prev_x = x;
        hl_prev_y = y;
        hl_prev_theta = theta;
        hl_prev_time = t;
        ...
        hl_vy_body_est = 0.0;
        hl_wz_est = 0.0;
        hl_vel_initialized = 1;
        return;
    }

    double dt = t - hl_prev_time;
    if (dt <= 1.0e-9) {
        return;
    }

    // Finite-difference world-frame velocity.
    double xdot_w = (x - hl_prev_x) / dt;
    double ydot_w = (y - hl_prev_y) / dt;
    double w_z    = wrap_to_pi_ctrl(theta - hl_prev_theta) / dt;

    // Convert world-frame velocity to body-frame velocity:
    // [vx_body; vy_body] = R(theta)^T [xdot_world; ydot_world]
    double c = cos(theta);
    double ss = sin(theta);
    hl_vx_body_est =  c*xdot_w + ss*ydot_w;
    hl_vy_body_est = -ss*xdot_w + c*ydot_w;
    hl_wz_est = w_z;
  ...
  for (j=0;j<4;j++)
  {
    ...
    if (ttime >= ts && ttime <= ts+tend) //ensures trajectory is provided after ts seconds
    {
      if (prev_step < step_no) //ensures values are set once per step
      {
        ...
        // Estimate actual high-level body-frame velocities from the measured pose.
        // These are not used by the current first-order filter, but they are
        // needed for fitting/validating a second-order high-level model.
        update_highlevel_velocity_estimate(x_c, y_c, theta, ttime);
       ...
       if (cbf_on == 1){
          ...
          // Keep logger values well-defined if the filter is disabled.
          cbf_log_u_nom[0] = vx;
          cbf_log_u_nom[1] = vy;
          cbf_log_u_nom[2] = omega;
        ...
        // data.txt columns:
        //  1      step
        //  2-4    safe/filtered command: vx, vy, omega
        //  5-7    nominal command before filtering
        //  8-10   backup command
        //  11     mu/interpolation parameter (-1 for bCBF)
        //  12     status flag
        //  13     QP/solve time in seconds
        //  14-16  estimated actual body-frame velocity: vx, vy, omega
        fprintf(fidC,
                "%d %.9f %.9f %.9f %.9f %.9f %.9f %.9f %.9f %.9f %.9f %d %.9e %.9f %.9f %.9f\n",
                step_no,
                vx, vy, omega,
                ...
                cbf_log_status,
                cbf_log_qp_time_sec,
                hl_vx_body_est, hl_vy_body_est, hl_wz_est);
