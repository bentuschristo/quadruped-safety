...
// =============================================================================
// LSE-aggregated Backup-CBF filter for the high-level quadruped model
// State:   X = [x, y, theta]
...
// and the terminal backup-set constraint into ONE smooth QP constraint.
// =============================================================================
...
static const double bcbf_kappa_lse = 20.0;  // larger -> closer to hard min over horizon terms
...
// LSE terms = obstacle margins at all backup samples + terminal backup-set margin.
// LSE aggregation reduces the horizon-sampled bCBF constraints to ONE QP row.
#define BCBF_N_LSE_TERMS (BCBF_N_OBS * BCBF_N_SAMPLES + 1)
                                   ...
                                   double b_cbf[BCBF_N_CBF_ROWS])
{
    ...
    // Terminal backup-set margin h_b(phi_b(T,x)) for C_B = {y >= backup_y}.
    double grad_zB[3] = {0.0, 1.0, 0.0};
    h_terms[term] = bcbf_h_backup(z[0], z[1]);
    grad_terms[term][0] = grad_terms[term][1] = grad_terms[term][2] = 0.0;
    for (int j = 0; j < 3; j++) {
        for (int m = 0; m < 3; m++) grad_terms[term][j] += grad_zB[m] * Phi[m][j];
    }
    term++;
    ...
    double  wx,     double  wy,     double  ww)
{
    ...
    double qp_t0 = cbf_now_sec(); //Timer start
    osqp_solve(bcbf_solver); // Solve QP
    cbf_log_qp_time_sec = cbf_now_sec() - qp_t0; // Timer end and log
