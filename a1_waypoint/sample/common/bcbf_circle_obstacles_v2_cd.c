...
// along the backup rollout, plus terminal clearance constraints for C_B.
// =============================================================================
...
static double bcbf_u_max[3] = {  2.0,  2.0,  2.0 };
static double bcbf_u_min[3] = { -2.0, -2.0, -2.0 };
...
// Total constraints = backup CBF rows + current command-state input-limit CBF rows.
                                   ...
                                   double b_cbf[BCBF_N_ROWS])
{
    ...
    for (int k = 0; k < BCBF_N_SAMPLES; k++) {
        for (int i = 0; i < BCBF_N_OBS; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                for (int m = 0; m < 6; m++) grad0[j] += grad_z[m] * Phi[m][j];
            }
            double hval = bcbf_h_obs(i, z[0], z[1]);
            bcbf_push_aug_row(grad0, z0, hval, bcbf_obs[i].alpha,
                              A_cbf[row], &b_cbf[row]);
            row++;
        }
        ...
        if (k < BCBF_N_SAMPLES - 1) {
            ...
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 6; j++) Phi[i][j] += dt * Phidot[i][j];
            }
        }
    }

    // Terminal backup-set constraint: LSE over enlarged-obstacle margins.
    const double kappa = bcbf_kappa_lse_B;
    double hvals[BCBF_N_OBS];
    double zmax = -1.0e300;
    for (int i = 0; i < BCBF_N_OBS; i++) {
    ...
    for (int i = 0; i < BCBF_N_OBS; i++) {
        double wi = exp((-kappa * hvals[i]) - zmax);
        sumexp += wi;
        grad_z_lse[0] += wi * 2.0 * (z[0] - bcbf_obs[i].cx);
        grad_z_lse[1] += wi * 2.0 * (z[1] - bcbf_obs[i].cy);
    }
    double hB_lse;
    if (sumexp <= 0.0 || !isfinite(sumexp)) {
        int imin = 0;
        ...
        grad_z_lse[0] = 2.0 * (z[0] - bcbf_obs[imin].cx);
        grad_z_lse[1] = 2.0 * (z[1] - bcbf_obs[imin].cy);
    } else {
    ...
    bcbf_push_aug_row(grad0B, z0, hB_lse, bcbf_alpha_B, A_cbf[row], &b_cbf[row]);
    row++;

    double A_input[6][3], b_input[6];
    bcbf_push_input_limit_rows(z0, A_input, b_input);
    for (int r = 0; r < 6; r++) {
        for (int j = 0; j < 3; j++) A_cbf[row][j] = A_input[r][j];
        b_cbf[row] = b_input[r];
        row++;
    }
}
