...
//      u_dot = -A u + A uhat
//
// Y-axis backup under control dynamics:
...
// Upward backup world-frame speed used in the augmented backup rollout.
static const double bcbf_backup_vup = 0.30;

// Terminal command tracking tolerance used only when BCBF_USE_AUGMENTED_TERMINAL_SET = 1.
// This requires u(T) to be close to the desired Y-backup body-frame command.
...
{
    // Y-axis/upward backup in surrogate-input space.
    // Desired backup world velocity is [0, v_up], converted to body frame:
    //   vx_B = v_up sin(theta), vy_B = v_up cos(theta), omega_B = 0.
    // The actual command u tracks this through u_dot = -A u + A uhat_B.
    (void)x; (void)y;
    ub[0] = bcbf_backup_vup * sin(th);
    ub[1] = bcbf_backup_vup * cos(th);
    ub[2] = 0.0;

    // Keep backup command inside admissible command limits.
    ub[0] = bcbf_clip(ub[0], bcbf_u_min[0], bcbf_u_max[0]);
    ub[1] = bcbf_clip(ub[1], bcbf_u_min[1], bcbf_u_max[1]);
    ub[2] = bcbf_clip(ub[2], bcbf_u_min[2], bcbf_u_max[2]);
}
