...
// =============================================================================
// Closed-form LSE Backup-CBF filter with EXACT FIRST-ORDER CONTROL DYNAMICS and AUGMENTED BACKUP
...
//
// Implementation note:
//      The first-order control dynamics are propagated with the exact
//      zero-order-hold solution, not Euler integration:
//          u_next = exp(-A dt) u + (I - exp(-A dt)) uhat.
//      For the zero-surrogate backup uhat_B = 0, this becomes:
//          u_next = exp(-A dt) u.
//
// IMPORTANT:
...
static inline double bcbf_cd_exact_update_scalar(double u, double uhat,
                                                 double a, double dt)
{
    // Exact zero-order-hold solution of u_dot = -a u + a uhat
    // over one sample, assuming uhat is constant on [t_k, t_k + dt].
    if (dt <= 0.0 || !isfinite(dt)) return u;
    if (a <= 1e-12 || !isfinite(a)) {
        // Degenerate case: no low-pass dynamics. Keep state unchanged.
        return u;
    }
    double c = exp(-a * dt);
    return c * u + (1.0 - c) * uhat;
}
                                 ...
                                 double H[BCBF_N_BACKUP_TERMS])
{
    ...
    for (int k = 0; k < BCBF_N_SAMPLES; k++) {
        ...
        if (k < BCBF_N_SAMPLES - 1) {
            ...
            // Original-state rollout is still first-order over the horizon sample,
            // but the control-dynamics part is propagated exactly.
            double xdot = c * z[3] - s * z[4];
            double ydot = s * z[3] + c * z[4];
            double thdot = z[5];

            z[0] += dt * xdot;
            z[1] += dt * ydot;
            z[2] = bcbf_wrap_pi(z[2] + dt * thdot);

            // Exact zero-surrogate backup: uhat_B = 0.
            z[3] = bcbf_cd_exact_update_scalar(z[3], 0.0, bcbf_cd_a[0], dt);
            z[4] = bcbf_cd_exact_update_scalar(z[4], 0.0, bcbf_cd_a[1], dt);
            z[5] = bcbf_cd_exact_update_scalar(z[5], 0.0, bcbf_cd_a[2], dt);
        }
    }
...
//            bcbf_cd_u_state[0], bcbf_cd_u_state[1], bcbf_cd_u_state[2]);
    ...
    // Exact zero-order-hold update of actual input state:
    //      u_dot = -A u + A uhat
    //      u_next = exp(-A dt) u + (I - exp(-A dt)) uhat
    // This replaces the previous Euler integration step.
    for (int j = 0; j < 3; j++) {
        bcbf_cd_u_state[j] = bcbf_cd_exact_update_scalar(
            bcbf_cd_u_state[j], uhat[j], bcbf_cd_a[j], bcbf_cd_dt);
