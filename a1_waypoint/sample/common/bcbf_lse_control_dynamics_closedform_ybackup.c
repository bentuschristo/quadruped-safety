...
//
// Based on the user's bcbf_lse_circle_obstacles.c upward/Y-axis backup-set setup.
// =============================================================================
...
// Backup set: upper refuge half-plane y >= bcbf_backup_y.
// The backup controller drives upward in world coordinates.
static const double bcbf_backup_y = 1.5;
static const double bcbf_alpha_B = 0.8;
static const double bcbf_kappa_lse   = 50.0;

// Backup controller: world-frame upward escape velocity [0, v_up].
...
static const double bcbf_alpha0_hocbf = 1.2;
...
#define BCBF_N_BACKUP_TERMS (BCBF_N_OBS * BCBF_N_SAMPLES + 1)
...
static inline void bcbf_backup_controller(double x, double y, double th, double ub[3])
{
    ...
    // Drive upward in world coordinates toward the backup half-plane y >= bcbf_backup_y.
    ...
    // Keep heading unchanged by default.
...
{
    (void)x;
    return y - bcbf_backup_y; // backup set y >= bcbf_backup_y iff h_B >= 0
}
                                       ...
                                       double gradH[BCBF_N_BACKUP_TERMS][3])
{
    ...
    // Terminal backup-set margin h_B(phi_B(T,x)) for C_B = {y >= bcbf_backup_y}.
    double grad_zB[3] = {0.0, 1.0, 0.0};
    H[term] = bcbf_h_backup(z[0], z[1]);
    gradH[term][0] = gradH[term][1] = gradH[term][2] = 0.0;
    for (int j = 0; j < 3; j++) {
        for (int m = 0; m < 3; m++) {
            gradH[term][j] += grad_zB[m] * Phi[m][j];
        }
    }
    term++;
