...
if exist(ctrl_file, 'file') == 2
    ...
    if size(ctrl_data, 2) >= 12
        ...
        %% ================= Figure 3: QP / solve computation time =================
        % Newer controller logs have a 13th column:
        % qp_time_sec = measured solve time in seconds.
        % For bCBF/bCBF-LSE this is OSQP solve time.
        % For OI this is scalar interpolation solve time.
        % For blending this is usually zero because there is no QP solve.
        if size(ctrl_data, 2) >= 13
            qp_time_sec = ctrl_data(:,13);
            qp_time_ms = 1000.0 * qp_time_sec;
            qp_valid = qp_time_sec(isfinite(qp_time_sec));
            qp_avg = mean(qp_valid);
            qp_max = max(qp_valid);

            hh3 = figure(3);
            clf;
            set(hh3, 'PaperUnits', 'inches');
            set(hh3, 'PaperPosition', PaperPosition);
            set(hh3, 'PaperSize', PaperSize);

            plot(cstep, qp_time_ms, 'm', 'LineWidth', 1.8); hold on;
            grid on;
            xlabel('step no', 'Fontsize', Fontsize);
            ylabel('solve time (ms)', 'Fontsize', Fontsize);
            title('Computation Time per Control Step', 'Fontsize', Fontsize+1);
            ax = gca; ax.FontSize = Fontsize;

            % Add average/max annotation in seconds and milliseconds.
            text(0.02, 0.95, sprintf('avg: %.3g ms (%.3g s)\nmax: %.3g ms (%.3g s)', ...
                1000.0*qp_avg, qp_avg, 1000.0*qp_max, qp_max), ...
                'Units', 'normalized', ...
            ...
            y_max = max(qp_time_ms(isfinite(qp_time_ms)));
            if ~isempty(y_max) && isfinite(y_max) && y_max > 0
                ylim([-0.06*y_max, 1.10*y_max]);
            end

            string3 = [path, 'computation_time_solve'];
            if (print_pdf == 1)
                print(hh3, '-dpdf', string3);
            end
        else
            warning('Control log %s has no qp_time_sec column. Expected column 13 for Figure 3.', ctrl_file);
        end
    else
