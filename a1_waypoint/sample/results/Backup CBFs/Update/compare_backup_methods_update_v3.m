%% compare_backup_methods_update_v4.m
...
%   col 13: solve/filter time in seconds
%
% Notes:
...
fig_dir = fullfile(root_dir, 'figures_compare_methods_v4_separate');
...
for s = 1:numel(scenarios)
    ...
    xlabel('step', 'FontSize', Fontsize);
    ylabel('solve / filter time (ms)', 'FontSize', Fontsize);
    title(sprintf('%s: computation time comparison', scenario_titles{s}), ...
        'FontSize', Fontsize + 2, 'FontWeight', 'bold');

    if has_time
        legend('Location', 'best', 'FontSize', 9);
    else
        text(0.1, 0.5, 'No timing data found. Need column 13 in data.txt.', ...
            'Units', 'normalized', 'FontSize', Fontsize);
    end

    save_if_requested(fig, fig_dir, save_figures, sprintf('timing_%s', sanitize_name(scenarios{s})));
end
