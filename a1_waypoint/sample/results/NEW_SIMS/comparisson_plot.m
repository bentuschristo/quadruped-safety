...
% All plots are displayed as tabs in one figure window.
% Saved files are exported from the individual tab axes.
...
% Optional zoom-in inset for the full safety-filter computation-time plot.
% Position is normalized figure coordinates: [x y width height].
full_filter_zoom_enabled = true;
full_filter_zoom_position = [0.55, 0.55, 0.34, 0.28];
full_filter_zoom_xlim = [0, 100];  % set e.g. [0, 2000], or [NaN, NaN] for auto
full_filter_zoom_ylim = [0, 0.25];  % set e.g. [0, 5], or [NaN, NaN] for auto
full_filter_zoom_title = 'zoom';
...
obs = [ ...
    -2.0,  1.0, 0.35, 0.45, 0.70;
    -4.0, -1.0, 0.35, 0.45, 0.70;
    -2.0, -1.0, 0.35, 0.45, 0.70;
    -4.0,  1.0, 0.35, 0.45, 0.70
];
...
CS_color       = [0.80 0.95 0.75];   % light green, safe set C_S
CB_color       = [0.75 0.93 1.00];   % light blue, backup set C_B
...
for s = 1:numel(scenarios)
    ...
    tab = uitab(tab_group, 'Title', sprintf('%s - Trajectory', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('trajectory_%s', sanitize_name(scenarios{s})));

    %% 2) Filtered vx
    tab = uitab(tab_group, 'Title', sprintf('%s - v_x', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('vx_%s', sanitize_name(scenarios{s})));

    %% 3) Filtered vy
    tab = uitab(tab_group, 'Title', sprintf('%s - v_y', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('vy_%s', sanitize_name(scenarios{s})));

    %% 4) Filtered omega
    tab = uitab(tab_group, 'Title', sprintf('%s - omega', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('omega_%s', sanitize_name(scenarios{s})));

    %% 5) Interpolation / blending mu only
    tab = uitab(tab_group, 'Title', sprintf('%s - mu interp', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('mu_interp_%s', sanitize_name(scenarios{s})));

    %% 6) Relaxation mu for closed-form bCBF only
    tab = uitab(tab_group, 'Title', sprintf('%s - mu relax', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('mu_relax_%s', sanitize_name(scenarios{s})));

    %% 7) Computation / solver time
    tab = uitab(tab_group, 'Title', sprintf('%s - Solver time', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('solver_timing_%s', sanitize_name(scenarios{s})));

    %% 8) Full safety-filter time (new logs, column 14)
    tab = uitab(tab_group, 'Title', sprintf('%s - Full filter time', scenario_titles{s}));
    ax = axes('Parent', tab, 'Position', [0.08 0.10 0.88 0.84]);
    axes(ax);
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');
    ...
    if has_full_time
        ...
        if full_filter_zoom_enabled
            add_full_filter_time_zoom_inset_sim(tab, D, s, methods, colors, linestyles, ...
                Linewidth, full_filter_zoom_position, full_filter_zoom_xlim, ...
                full_filter_zoom_ylim, full_filter_zoom_title);
        end
    else
    ...
    save_if_requested(ax, fig_dir, save_figures, sprintf('full_filter_timing_%s', sanitize_name(scenarios{s})));
end
...
function add_full_filter_time_zoom_inset_sim(parent_tab, D, scenario_idx, methods, colors, linestyles, Linewidth, inset_position, zoom_xlim, zoom_ylim, zoom_title)
    ax = axes('Parent', parent_tab, 'Position', inset_position); %#ok<LAXES>
    hold(ax, 'on'); grid(ax, 'on'); box(ax, 'on');

    for m = 1:numel(methods)
        ctrl = D(scenario_idx,m).ctrl;
        if isempty(ctrl)
            continue;
        end
        ...
        plot(ax, step, full_filter_time_ms, ...
            'LineWidth', max(0.8, 0.75*Linewidth), ...
            'Color', colors{m}, ...
            'LineStyle', linestyles{m}, ...
            'HandleVisibility', 'off');
    end

    if nargin >= 8 && numel(zoom_xlim) == 2 && all(isfinite(zoom_xlim))
        xlim(ax, zoom_xlim);
    end
    if nargin >= 9 && numel(zoom_ylim) == 2 && all(isfinite(zoom_ylim))
        ylim(ax, zoom_ylim);
    end

    title(ax, zoom_title, 'FontSize', 9);
    xlabel(ax, 'step', 'FontSize', 8);
    ylabel(ax, 'ms', 'FontSize', 8);
    ax.FontSize = 8;
end
...
function save_if_requested(ax, fig_dir, save_figures, base_name)
    if ~save_figures
        return;
    end
    ...
    exportgraphics(graphics_handle, out_png, 'Resolution', 300);
    exportgraphics(graphics_handle, out_pdf, 'ContentType', 'vector');
