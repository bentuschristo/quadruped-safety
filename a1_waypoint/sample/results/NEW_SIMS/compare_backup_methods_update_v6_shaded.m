...
%   [x_center, y_center, real_radius, barrier_radius]
% The red annulus is the buffer zone between real_radius and barrier_radius.
% Change these numbers to match your simulation setup.
obs = [ ...
    -2.0,  1.0, 0.35, 0.45;
    -4.0, -1.0, 0.35, 0.45;
    -2.0, -1.0, 0.35, 0.45;
...
% Outer safe-set rectangle C_S and inner backup-set rectangle C_B.
% Set draw_region_shading=false if you only want obstacle circles.
CS_box = [-5.2, -0.8, -2.3, 2.3];   % [xmin xmax ymin ymax]
CB_box = [-4.7, -1.3, -1.8, 1.8];   % [xmin xmax ymin ymax]

% Shading colors and transparency. Change freely.
CS_color       = [0.80 0.95 0.75];   % light green
CB_color       = [0.75 0.93 1.00];   % light blue
obstacle_color = [0.85 0.85 0.85];   % gray real obstacle
buffer_color   = [1.00 0.20 0.20];   % red buffer ring
CS_alpha       = 0.35;
CB_alpha       = 0.35;
obstacle_alpha = 0.85;
buffer_alpha   = 0.65;
...
for s = 1:numel(scenarios)
    ...
    if draw_region_shading
        plot_region_shading(CS_box, CB_box, CS_color, CB_color, CS_alpha, CB_alpha);
    end
    if draw_obstacles
        plot_obstacle_shading(obs, obstacle_color, buffer_color, obstacle_alpha, buffer_alpha, show_obstacle_labels);
    end
...
function plot_region_shading(CS_box, CB_box, CS_color, CB_color, CS_alpha, CB_alpha)
...
% Boxes are [xmin xmax ymin ymax]. Patches are hidden from legend.
    ...
    % Optional labels. Move/delete these if they overlap your trajectories.
    text(CS_box(2), CS_box(4), '$ar{ar C}_S$', ...
...
function plot_obstacle_shading(obs, obstacle_color, buffer_color, obstacle_alpha, buffer_alpha, show_labels)
% Draw real obstacles and red buffer annuli.
% obs columns: [x_center, y_center, real_radius, barrier_radius].
% If only three columns are supplied, the third column is used as real_radius
% and the barrier radius is set equal to the real radius.
    ...
    for i = 1:size(obs,1)
        ...
        if r_barrier < r_real
            warning('Obstacle %d has barrier radius smaller than real radius. Swapping.', i);
            tmp = r_real; r_real = r_barrier; r_barrier = tmp;
        end

        % Red buffer zone: annulus between real obstacle and barrier radius.
        if r_barrier > r_real
            x_outer = ox + r_barrier*cos(th);
            y_outer = oy + r_barrier*sin(th);
        ...
        % Barrier-limit outline.
        if r_barrier > r_real
            plot(ox + r_barrier*cos(th), oy + r_barrier*sin(th), ...
